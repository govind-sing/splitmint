        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-4xl p-8 shadow-sm border border-slate-100 space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-end ml-1">
                ...
                <span className="text-[10px] font-bold text-slate-300 italic">
                  {participants.length + 1}/4 Slots
                </span>
              </div>

              {/* You */}
              <div className="flex items-center gap-3 p-4 bg-emerald-50 rounded-2xl border border-emerald-100 animate-in fade-in duration-500">
                <div className="w-8 h-8 rounded-xl bg-emerald-500 flex items-center justify-center text-white text-xs font-black">
                  Y
                </div>
                ...
                {participants.map((p, index) => (
                  <div key={index} className="flex gap-2 animate-in slide-in-from-left-2 duration-300">
                    ...
                    <button
                      ...
                      className="p-3 text-rose-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all active:scale-90 cursor-pointer"
                    >
              ...
              {participants.length < 3 && (
                <button
                  ...
                  className="w-full py-3 border-2 border-dashed border-slate-100 rounded-2xl text-[10px] font-black text-slate-400 uppercase tracking-widest hover:border-emerald-500/30 hover:text-emerald-500 hover:bg-emerald-50/30 transition-all active:scale-[0.98] cursor-pointer"
                >
                  + Add Another Guest
                </button>
              )}
          ...
          <div className="space-y-3 pt-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full py-5 rounded-2xl font-black text-sm text-white transition-all active:scale-95
              ${loading
                ? "bg-emerald-400 cursor-not-allowed"
                : "bg-emerald-500 hover:bg-emerald-600 shadow-xl shadow-emerald-100 cursor-pointer"
              }`}
            >
            ...
            <button
              ...
              className="w-full py-4 text-slate-400 hover:text-slate-600 font-bold text-xs uppercase tracking-widest transition-colors cursor-pointer"
            >
              Cancel
            </button>
