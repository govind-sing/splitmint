    ...
    const fetchParticipants = async () => {
      try {
        ...
        if (res.data.participants.length > 0) {
          setPayerId(res.data.participants[0].id);
        }
      } catch (err) {
    ...
    <div className="min-h-screen bg-[#F9FBFC] text-slate-900 font-sans p-4 sm:p-8 flex flex-col items-center">
      ...
      <div className="w-full max-w-lg">
        ...
        <button
          ...
          className="mb-6 flex items-center gap-2 text-slate-400 hover:text-slate-600 transition-colors font-bold text-sm group cursor-pointer"
        >
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-4xl p-8 shadow-sm border border-slate-100 space-y-6">
          ...
          <div className="space-y-4">
            ...
            {splitMode !== 'equal' && (
              <div className="bg-slate-50 rounded-3xl p-6 border border-slate-100 animate-in fade-in slide-in-from-top-2">
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 italic text-center">
                  Enter {splitMode} for each
                </p>
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full py-5 rounded-2xl font-black text-sm transition-all active:scale-95 text-white
            ${loading
